...
/**
 * Patches a member's Google Wallet loyalty object so their card shows the
 * current stamp count.  Lives here rather than inline in the route handler so
 * server-side callers (the referral reward in app/actions/join.ts) can reach
 * the same code path without an HTTP round trip to our own API.
 *
 * Never throws: a member who never added a pass is the normal case, not an
 * error worth failing the caller's mutation over.
 */
export async function updateWalletPass({
  ...
  businessId,
  brandColor,
}: WalletUpdateInput): Promise<void> {
